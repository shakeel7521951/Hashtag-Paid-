...
const OmniChannel = () => {
  const licensingRef = useRef();
  const location = useLocation();

  useEffect(() => {
    if (location.hash === "#licensing") {
      licensingRef.current?.scrollIntoView({ behavior: "smooth" });
    }
  }, [location.hash]);
    ...
    <div className="bg-[#F2F0EB] flex flex-col sm:flex-row rounded-xl px-5 sm:px-10 py-6 sm:py-10 gap-5 sm:gap-10">
      ...
      <div className="w-full sm:w-[50%] mt-10">
        ...
        <p className="font-customLight my-5 text-xl">
          License creator content to use across all your marketing channels.
          Whether it’s out-of-home, digital ads, print, or in-flight
          entertainment, create a truly omni-channel creator marketing
          experience.
        </p>
        <div className="flex flex-col gap-3">
          ...
          <div className="flex items-center gap-2">
            <img src={tick} alt="Tick" />
            <p className="text-md">
              No guesswork with a proprietary pricing algorithm based on first
              party data
            </p>
          </div>
          <div className="flex items-center gap-2">
            <img src={tick} alt="Tick" />
            <p className="text-md">
              Lock in your rates before the campaign, or buy them after your
              content goes lives
            </p>
          </div>
        </div>
